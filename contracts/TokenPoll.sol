...
contract TokenPoll is Ownable, ReentrancyGuard, DevRequire, QuadraticVoting {
  ...
  uint public constant maxTimeBetweenFundingRounds = 1000 days;
  ...
  function initialize(address _icoToken, address _stableCoin) external onlyOwner nonReentrant {
    devRequire(address(icoCoin) == address(0x0), "Can only set this once");
    devRequire(address(stableCoin) == address(0x0), "Can only set this once");
  ...
  function pullFundsAndDisburseRound1(address fundsOrigin, uint fundsBalance) onlyOwner nonReentrant external {
    ...
    devRequire(address(icoCoin) != address(0x0), "Voter coins not set");  
    devRequire(address(stableCoin) != address(0x0), "Stablecoin not set");

    // Parties agree on how much to transfer 
    devRequire(stableCoin.allowance(fundsOrigin, address(this)) == fundsBalance, "Parties do not agree on amount to exchange");
