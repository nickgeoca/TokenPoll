...
contract TokenPoll is Ownable, ReentrancyGuard, DevRequire, QuadraticVoting {
  ...
  function pullFundsAndDisburseRound1(address fundsOrigin, uint fundsBalance) onlyOwner nonReentrant external {
    devRequire(projectWallet != address(0x0), "Project wallet address is empty");

    // Get funds, then send round 1
    devRequire(stableCoin.transferFrom(fundsOrigin, address(this), fundsBalance), "Mismatch on expected funds to receive");
    devRequire(stableCoin.transfer(projectWallet, roundOneFunding), "Failed to disburse round 1 amount");
